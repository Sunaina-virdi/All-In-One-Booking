    ...
    if (id) {
      axios.get('/bookings').then(response => {
        ...
        if (foundBooking) {
          ...
          // console.log("userrr:",foundBooking.user.email);
                    // Send confirmation email once the booking is found
          // sendConfirmationEmail(foundBooking);
        }
      });
    }
  }, [id]);
...
const handleAddReview = (updatedReviews) => {
  ...
  //   console.log("sendConfirmationEmail called with bookingDetails:", bookingDetails);
  
  //   const { name, email, phone, checkIn, checkOut, place, price } = bookingDetails;
  ...
  //   console.log("Place title:", place?.title || "No title");
  //   console.log("Price:", price);
  
  //   // Sending booking confirmation email to the user
  //   axios
  //     .post("http://localhost:4000/send-email", {
  //       to: bookingDetails.user?.email || "No email provided", // Use bookingDetails.user.email
  //       subject: "Booking Confirmation",
  //       text: `Dear ${name},\n\nYour booking has been confirmed for the place "${place.title}".\nPhone: ${phone}\nCheck-in: ${format(
  //         new Date(checkIn),
  //         "yyyy-MM-dd"
  //       )}\nCheck-out: ${format(new Date(checkOut), "yyyy-MM-dd")}\nTotal Price: Rs ${price}\n\nThank you for booking with us!`,
  //       html: `<p>Dear <strong>${name}</strong>,</p><p>Your booking has been confirmed for the place <strong>"${place.title}"</strong>.</p><p>Phone: ${phone}</p><p>Check-in: ${format(
  //         new Date(checkIn),
  //         "yyyy-MM-dd"
  //       )} &rarr; Check-out: ${format(new Date(checkOut), "yyyy-MM-dd")}</p><p><strong>Total Price: Rs ${price}</strong></p><p>Thank you for booking with us!</p>`,
  //     })
  //     .then((response) => {
  ...
  //     })
  //     .catch((error) => {
  //       console.error("Error sending confirmation email via Axios POST:");
  //       console.error("Error message:", error.message);
  //       console.error("Error response:", error.response?.data || "No error response data");
  //     });
  // };
